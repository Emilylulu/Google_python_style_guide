...
多数情况下可以使用。
#### 2.12.1 定义
可以在函数的参数表的最后为变量指定值，例如``def foo(a,b=0)``，如果``foo``被调用时只传了一个变量，那``b``就被赋值为0。如果``foo``被调用时只传了两个变量，那``b``的值就是第二个变量的值。
#### 2.12.2 优点
通常有些函数有很多默认值，而且这些默认值很少被覆盖。默认参数就可以更简单的实现这种情况而不需要为特殊情况（即更改默认值）重新定义很多函数。python不支持overload 重载函数，而默认函数值可以达到类似于重载的效果。
#### 2.12.3 缺点
...
### 2.14 布尔量评估
#### 2.14.1 定义
在python中，具体的值被当做``False``。所有被当做是“空”的值也当做flase。所以，``0,None,[],{},''``都是false。
#### 2.14.2 优点
在条件中使用布尔类型不容易出错，通常也比较快。
#### 2.14.3 缺点
对于C或C++开发者来说可能很奇怪。
#### 2.14.4 使用
